...
To install rprimer, start R (version 4.6.0 or higher), and enter the following code:
...
The alignment must be imported to R using the `readDNAMultipleAlignment()` function from MultipleAlignment [@MultAlign]. The input file can contain from one to several thousand sequences. 
...
It is also possible to mask specific sequences by using `rowmask()`. Gap-rich positions can be hidden with `maskGaps()` [@MultAlign].
